...
if (isset($_GET['article_id']) && isnum($_GET['article_id'])) {
	...
	$result = dbquery("SELECT article_cat_id, article_cat_name, article_cat_description FROM ".DB_ARTICLE_CATS." WHERE ".groupaccess('article_cat_access')." AND article_cat_parent='0' ORDER BY article_cat_name");//subcategories
	...
	if ($rows) {
		...
		while ($data = dbarray($result)) {
			...
			echo subcats($data['article_cat_id'], $type="2");//subcategories
      echo "</td>\n";
			$counter++;
		}
		echo "</tr>\n</table>\n";
	...
	$res = 0;
	$result = dbquery("SELECT article_cat_id, article_cat_name, article_cat_sorting, article_cat_access FROM ".DB_ARTICLE_CATS." WHERE article_cat_id='".$_GET['cat_id']."'");//subcategories
	...
	if (dbrows($result) != 0) {
		...
		if (checkgroup($cdata['article_cat_access'])) {
			...
			echo subcats($cdata['article_cat_id'], $type="1");//subcategories
