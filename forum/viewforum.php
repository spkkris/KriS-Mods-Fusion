...
// Start. Tylko wstawiæ
$a_result = dbquery("SELECT * FROM ".DB_FORUMS." f LEFT JOIN ".DB_USERS." u on f.forum_lastuser=u.user_id WHERE forum_parent='".$_GET['forum_id']."'");
...
// Start. Podmienione za:
// $caption = $fdata['forum_cat_name']." :: ".$fdata['forum_name'];
if ($fdata['forum_parent'] != 0 ) {
$sub_data = dbarray(dbquery("SELECT forum_name FROM ".DB_FORUMS." WHERE forum_id='".$fdata['forum_parent']."'"));
	...
	if ($thread_ids) {
		...
		if (dbrows($result)) {
			while ($pdata = dbarray($result)) {
				$result2 = dbquery("UPDATE ".DB_USERS." SET user_posts=user_posts-".$pdata['num_posts'].", points_normal=points_normal-".points('f')." WHERE user_id='".$pdata['post_author']."'");
