		...
		if (dbrows($result)) {
			while ($pdata = dbarray($result)) {
				$result2 = dbquery("UPDATE ".DB_USERS." SET user_posts=user_posts-".$pdata['num_posts']." WHERE user_id='".$pdata['post_author']."'");
						dbquery("UPDATE ".DB_USERS." SET points_normal=IF(points_normal<".(points('f') * $pdata['num_posts']).",0,points_normal-".(points('f') * $pdata['num_posts']).") WHERE user_id='".$pdata['post_author']."'");
...
if (iMEMBER && $can_reply && !$fdata['thread_locked'] && isset($_POST['postquickreply'])) {
	...
	if ($message != "") {
		...
		if (!flood_control("post_datestamp", DB_POSTS, "post_author='".$userdata['user_id']."'")) {
			...
			$result = dbquery("INSERT INTO ".DB_POSTS." (forum_id, thread_id, post_message, post_showsig, post_smileys, post_author, post_datestamp, post_ip, post_edituser, post_edittime, useragent) VALUES ('".$fdata['forum_id']."', '".$_GET['thread_id']."', '$message', '$sig', '$smileys', '".$userdata['user_id']."', '".time()."', '".USER_IP."', '0', '0', '".stripinput(trim(eregi_replace(" +", " ",$_SERVER['HTTP_USER_AGENT'])))."')");
...
if ($rows != 0) {
    ...
    if(!$agent['system']) {
      ...
      echo "<div style='float:right' class='small'><img src='".INCLUDES."useragent/images/icon_".$agent['system_icon'].".png' border='0' title='".$agent['system']."' style='border:0;vertical-align:middle'/> <img src='".INCLUDES."useragent/images/icon_".$agent['browser_icon'].".png' border='0' title='".$agent['browser']."' style='border:0;vertical-align:middle'/> <a href='#post_".$data['post_id']."' name='post_".$data['post_id']."' id='post_".$data['post_id']."'>#".($current_row+$_GET['rowstart'])."</a>";
    ...
    echo "<img src='".IMAGES."avatars/noav.png' alt='".$locale['567']."' /><br /><br />\n";
    }
    		if ($data['user_status'] == 1) {

	echo "<span class='alt'><b>ZBANOWANY</b></span><br>";
	  ...
	  echo "<span class='small'><strong>Liczba pkt:</strong> ".show_points($data['user_id'])."</span><br />\n";
		echo "<span class='small'><strong>Ranga:</strong> ".check_rang($data['user_id'])."</span><br />\n";
    echo "<span class='small'><b><a href='view_post_helping_user.php?action=view_help&user_id=".$data['user_id']."'><span class='alt'>".$locale['499']."</span></a></b> <span class='alt'>".number_format($data['user_help_point'])."</span></span><br>";
		echo "<span class='small'><strong>".$locale['504']."</strong> ".showdate("%d.%m.%y", $data['user_joined'])."</span><br />\n";
		if ($data['post_help'] > 0) {
		echo "<br /></td>\n<td valign='top' class='tbl1' style='background-color: #FFFF99'>\n";
		} else {
...
echo "<span style='font-size: 9px; color: green;'><b>PW od moderatora:</b><br> <ol><li>".stripslash($r['notka'])." - ".$r['user_name']." ".strftime("%d.%m - %H:%M", $r['time'])."</ol></li></span>";
	}}	
		...
		if (array_key_exists("user_web", $data) && $data['user_web']) {
			...
			echo "<a href='".$urlprefix."".$data['user_web']."' target='_blank'><img src='".THEME."forum/web.gif' alt='".$data['user_web']."' style='border:0;vertical-align:middle' /></a> ";
		}
		if (iMEMBER && $data['user_id']!=$userdata['user_id']) {
			echo "<a href='".BASEDIR."user_center/messages.php?msg_send=".$data['user_id']."'><img src='".THEME."forum/pm.gif' alt='".$locale['572']."' style='border:0;vertical-align:middle' /></a>\n";
		        echo "<a href='".BASEDIR."spam.php?post_id=".$data['post_id']."&forum_id=".$data['forum_id']."&thread_id=".$data['thread_id']."'><img src='".THEME."forum/spam.gif' style='border:0px;vertical-align:middle'/></a>\n";
		}
		echo "</div>\n<div style='float:right' class='small'>";
		if (iMEMBER && ($can_post || $can_reply)) {
         ...
         echo "<a href='post.php?action=help&forum_id=".$data['forum_id']."&thread_id=".$data['thread_id']."&post_id=".$data['post_id']."'><img src='".THEME."forum/help.gif' style='border:0px;vertical-align:middle'/></a>\n";
             }			  
				echo "<a href='post.php?action=reply&amp;forum_id=".$data['forum_id']."&amp;thread_id=".$data['thread_id']."&amp;post_id=".$data['post_id']."&amp;quote=".$data['post_id']."'><img src='".THEME."forum/quote.gif' alt='".$locale['569']."' style='border:0px;vertical-align:middle' /></a>\n";
		if (iMOD || ($lock_edit && $last_post['post_id'] == $data['post_id'] && $userdata['user_id'] == $data['post_author']) || (!$lock_edit && $userdata['user_id'] == $data['post_author'])) {
					echo "<a href='post.php?action=edit&amp;forum_id=".$data['forum_id']."&amp;thread_id=".$data['thread_id']."&amp;post_id=".$data['post_id']."'><img src='".THEME."forum/edit.gif' alt='".$locale['568']."' style='border:0px;vertical-align:middle' /></a>\n";
				}
			} else {
				if (iMOD) {
					echo "<a href='post.php?action=edit&amp;forum_id=".$data['forum_id']."&amp;thread_id=".$data['thread_id']."&amp;post_id=".$data['post_id']."'><img src='".THEME."forum/edit.gif' alt='".$locale['568']."' style='border:0px;vertical-align:middle' /></a>\n";
  	   ...
  	   if(iMOD) {
        echo "<a href='".BASEDIR."notka.php?post_id=".$data['post_id']."&forum_id=".$data['forum_id']."&thread_id=".$data['thread_id']."'><img src='".THEME."forum/note.gif' style='border:0px;vertical-align:middle'/></a>\n";
        }
