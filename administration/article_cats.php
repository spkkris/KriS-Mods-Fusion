...
if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
	...
	if (isset($_POST['save_cat'])) {
		...
		$cat_parent = isnum($_POST['cat_parent']) ? $_POST['cat_parent'] : "0";//subcategories
		...
		if ($cat_name) {
			if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
				$result = dbquery("UPDATE ".DB_ARTICLE_CATS." SET article_cat_name='$cat_name', article_cat_description='$cat_description', article_cat_sorting='$cat_sorting', article_cat_parent='$cat_parent', article_cat_access='$cat_access' WHERE article_cat_id='".$_GET['cat_id']."'");//subcategories
        redirect(FUSION_SELF.$aidlink."&status=su");
			} else {
			$result = dbquery("INSERT INTO ".DB_ARTICLE_CATS." (article_cat_name, article_cat_description, article_cat_sorting, article_cat_parent, article_cat_access) VALUES ('$cat_name', '$cat_description', '$cat_sorting', '$cat_parent', '$cat_access')");//subcategories
	...
	echo "<table cellpadding='0' cellspacing='1' width='400' class='tbl-border center'>\n";
		$result = dbquery("SELECT article_cat_id, article_cat_name, article_cat_description, article_cat_access FROM ".DB_ARTICLE_CATS." WHERE article_cat_parent='0'  ORDER BY article_cat_name");//subcategories
	if (dbrows($result) != 0) {
		$i = 0;
		echo "<tr>\n";
		...
		while ($data = dbarray($result)) {
			...
			echo "</tr>\n";
			echo subcats($data['article_cat_id']);//subcategories
			$i++;
		}
		echo "</table>\n";
	} else {
