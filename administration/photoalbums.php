...
if (function_exists('gd_info')) {
	...
	if (isset($_POST['cancel'])) {
		...
		$album_sub = isNum($_POST['album_sub']) ? $_POST['album_sub'] : "";
		...
		if (is_uploaded_file($_FILES['album_pic_file']['tmp_name'])) {
			...
			if (!preg_match("/^[-0-9A-Z_\.\[\]]+$/i", $album_pic['name'])) {
				$error = 1;
		...
		if (!$error) {
			if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['album_id']) && isnum($_GET['album_id']))) {
				...
				$result = dbquery("UPDATE ".DB_PHOTO_ALBUMS." SET album_title='$album_title', album_description='$album_description',".(isset($album_thumb)?" album_thumb='$album_thumb',":"")." album_user='".$userdata['user_id']."', album_access='$album_access', album_order='$album_order', album_sub='$album_sub' WHERE album_id='".$_GET['album_id']."'");
				...
				$result = dbquery("INSERT INTO ".DB_PHOTO_ALBUMS." (album_title, album_description, album_thumb, album_user, album_access, album_order, album_datestamp, album_sub) VALUES ('$album_title', '$album_description', '$album_thumb', '".$userdata['user_id']."', '$album_access', '$album_order', '".time()."', '$album_sub')");
				$rowstart = $album_order > $settings['thumbs_per_page'] ? ((ceil($album_order / $settings['thumbs_per_page'])-1)*$settings['thumbs_per_page']) : "0";
		...
		echo "</td>\n</tr>
		<tr>
<td class='tbl'>".$locale['sub001']."</td>
<td class='tbl'><select name='album_sub' class='textbox' style='width:150px;'>";
$result3 = dbquery("SELECT * FROM ".DB_PREFIX."photo_albums");
echo "<option value=''>---</option>";
while($data3 = dbarray($result3)){
echo "<option value='".$data3['album_id']."'";
if($data3['album_id'] == $album_sub) echo " SELECTED";
echo ">".$data3['album_title']."</option>";
}
echo "</select></td>
</tr>\n<tr>\n";
	...
	if ($rows) {
		...
		while ($data = dbarray($result)) {
			...
			if ($data['album_thumb'] && file_exists(PHOTOS.$data['album_thumb'])){
				echo "<img src='".PHOTOS.rawurlencode($data['album_thumb'])."' alt='".$locale['461']."' style='border:0px' />";
			} else {
				echo $locale['461'];
			}
			echo "</a><br /><br />\n<span class='small'>".$up;
