	...
	if (isset($_POST['save'])) {
		...
		if ((isset($_POST['article_id']) && isnum($_POST['article_id'])) || (isset($_GET['article_id']) && isnum($_GET['article_id']))) {
			...
			opentable($locale['400']);
				...
				$result = dbquery("SELECT article_cat_id, article_cat_name FROM ".DB_ARTICLE_CATS." WHERE article_cat_parent='0' ORDER BY article_cat_name DESC");//subcategories
		$catlist = ""; $sel = "";
		while ($data = dbarray($result)) {
			...
			$catlist .= editlist_sub($data['article_cat_id']);//subcategories 
		}
...
//Subcategory begin
function editlist_sub($id) {
    global $sel;
	$result = dbquery("SELECT article_cat_id, article_cat_name FROM ".DB_ARTICLE_CATS." WHERE article_cat_parent='".$id."' ORDER BY article_cat_name DESC");
	$catlist = "";
	$k = dbrows($result);
	if ($k > 0) {
