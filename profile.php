...
if (!isset($_GET['group_id']) || !isnum($_GET['group_id'])) {
	...
	add_to_title($locale['global_200'].$locale['400'].$locale['global_201'].$user_data['user_name']);
	    //blokuj IP z profilu   
    if (isset($_GET['banuj']) == "ip") {
    opentable($locale['400'] ."- Zbanuj IP");
    if (iADMIN) {
          $result = dbquery("SELECT * FROM ".DB_USERS." WHERE user_id='".$_GET['lookup']."'");
          $user_data = dbarray($result);
          $result = dbquery("SELECT * FROM ".DB_BLACKLIST." WHERE blacklist_ip='".$user_data['user_ip']."'");
          $data = dbarray($result);
     
     
     if ($data['blacklist_ip'] != $user_data['user_ip']) {
       $result = dbquery("INSERT INTO ".DB_BLACKLIST." (blacklist_ip, blacklist_email, blacklist_reason) VALUES ('".$user_data['user_ip']."', '$".$user_data['user_email']."', 'Zbanowany z profilu')");
     
       echo
     "<div class='tbl'><div align='center'>IP użytkownika zostało zbanowane</div><br>
       IP użytkownika: <b>"
    .$user_data['user_ip'
    ]."</b><br>
       Powód zbanowania: <b>Zbanowany z profilu</b>"
    ;
     } else {
      echo "<div align='center'>To IP zostało już wcześniej zablokowane</div>";
      }
      } else {
      echo "Brak dostepu do funkcji";
      }
    closetable();
     }
    //blokuj IP z profilu - koniec
 ...
 echo "<td width='1%' class='tbl1' style='white-space:nowrap'>\n";
      ...
      if ($lastseen < 60) {
         $lastseen = "<img src='".IMAGES."online.png' alt='Online' title='Online'>";
      } elseif ($lastseen < 300)  {
         $lastseen = "<img src='".IMAGES."offline.png' alt='Wyszedł' title='Wyszedł'>";
      } else {
         $lastseen = "<img src='".IMAGES."offline.png' alt='Offline' title='Offline'>";
      }
   echo $lastseen."</span>\n";
   echo"</td>\n";
	...
	if (iADMIN && checkrights("M") && $user_data['user_id'] != $userdata['user_id']) {
		...
		if ($user_data['user_level'] < 102) {
			...
			echo "<a href='".ADMIN."members.php".$aidlink."&amp;step=ban&amp;act=on&amp;user_id=".$user_data['user_id']."&amp;status=1' onclick=\"return confirm('".$locale['413']."');\">".$locale['411']."</a> ::\n";
			    //blokuj IP z profilu
             echo "<a href='profile.php?lookup=".$user_data['user_id']."&banuj=ip' onclick=\"return confirm('Czy chcesz zbanować IP użytkownika?');\">Banuj IP</a> ::\n";
             //blokuj IP z profilu - koniec
