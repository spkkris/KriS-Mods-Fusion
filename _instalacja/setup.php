...
echo "<link rel='stylesheet' href='styles/setup_styles.css' type='text/css' />\n";
echo "</head>\n<body>\n";
...
if (isset($_POST['step']) && $_POST['step'] == "2") {
	...
	echo "<td class='tbl1' style='text-align:right'>".(is_writable("../infusions/rss_parser_panel/cache") ? "<span class='passed'>".$locale['023']."</span>" : "<span class='failed'>".$locale['024']."</span>")."</td>\n</tr>\n";
	echo "<tr>\n<td class='tbl1'>infusions/rss_parser_panel/cache</td>\n";
...
if (isset($_POST['step']) && $_POST['step'] == "4") {
	...
	if ($link = dbconnect($db_host, $db_user, $db_pass, $db_name)) {
      ...
      forum_parent MEDIUMINT( 8 ) NOT NULL DEFAULT '0',
			...
			album_sub MEDIUMINT( 7 ) NOT NULL,
			...
			post_help SMALLINT(5) UNSIGNED NOT NULL DEFAULT '0',
			PRIMARY KEY (post_id),
			KEY thread_id (thread_id),
			...
			register_pm_subject VARCHAR(200) NOT NULL DEFAULT '',
			register_pm_message TEXT NOT NULL
			) ".ENGINE.";");
			...
			thread_help1 SMALLINT(3) UNSIGNED NOT NULL DEFAULT '0',
			...
			user_help_point SMALLINT(5) UNSIGNED NOT NULL DEFAULT '0',
			...
			user_primarygroup VARCHAR(4) NOT NULL DEFAULT '',
			PRIMARY KEY (user_id),
			...
			$result = dbquery("DROP TABLE IF EXISTS ".$db_prefix."vip");
			$result = dbquery("CREATE TABLE ".$db_prefix."vip(
			numer_usera VARCHAR(50) NOT NULL DEFAULT '0',
			identyfikator VARCHAR(50) NOT NULL DEFAULT '0',
			numer_sms VARCHAR(50) NOT NULL DEFAULT '0',
			tresc_sms VARCHAR(50) NOT NULL DEFAULT '0',
			koszt VARCHAR(50) NOT NULL DEFAULT '0',
			koszt_brutto VARCHAR(50) NOT NULL DEFAULT '0'
			) ".ENGINE.";");
			
		  if (!$result) { $fail = true; }
			
			$result = dbquery("DROP TABLE IF EXISTS ".$db_prefix."rss_feeds");
			$result = dbquery("CREATE TABLE ".$db_prefix."rss_feeds(
			id int(5) unsigned NOT NULL auto_increment,
			name varchar(100) NOT NULL default '',
			file varchar(255) NOT NULL default '',
			type smallint(1) NOT NULL default '0',
			in_feeds_page tinyint(1) NOT NULL default '0',
			page_order int(5) unsigned NOT NULL default '0',
			num_items_center_panel int(5) unsigned NOT NULL default '0',
			num_items_side_panel int(5) unsigned NOT NULL default '0',
			num_items_feeds_page int(5) unsigned NOT NULL default '0',
			cache_limit int(10) unsigned NOT NULL default '0',
			new_window smallint(1) unsigned NOT NULL default '1',
			PRIMARY KEY  (id)
			) ".ENGINE.";");
      ...
      $result = dbquery("DROP TABLE IF EXISTS ".$db_prefix."forumlog");
      $result = dbquery("CREATE TABLE ".$db_prefix."forumlog(
		  log_id int(5) unsigned NOT NULL AUTO_INCREMENT,
      u_id smallint(6) unsigned NOT NULL,
      forum_id smallint(5) unsigned NOT NULL default '0',
      thread_id smallint(5) unsigned NOT NULL default '0',
      action TEXT NOT NULL,
      thread_subject varchar(100) default 'Deleted',
      ...
      log_ip varchar(20) NOT NULL default '0.0.0.0',
       PRIMARY KEY (log_id)
		  ...
		  $result = dbquery("DROP TABLE IF EXISTS ".$db_prefix."nicks_mod");
      $result = dbquery("CREATE TABLE ".$db_prefix."nicks_mod(
		  nicks_group_id VARCHAR(4) NOT NULL,
      nicks_color VARCHAR(7) NOT NULL,
      nicks_prefix VARCHAR(2) NOT NULL,
      nicks_styles VARCHAR(100) NOT NULL,
      PRIMARY KEY (nicks_group_id)
	    ) ".ENGINE.";");
	    
	     if (!$result) { $fail = true; }
	    
	    $result = dbquery("DROP TABLE IF EXISTS ".$db_prefix."eps_points");
      $result = dbquery("CREATE TABLE ".$db_prefix."eps_points(
      point_id smallint(5) unsigned NOT NULL,
      point_type varchar(30) NOT NULL,
      point_ammount smallint(5) NOT NULL,
      PRIMARY KEY  (point_id)
       ) ".ENGINE.";");
  
      if (!$result) { $fail = true; }
  
     $result = dbquery("DROP TABLE IF EXISTS ".$db_prefix."eps_rangs");
     $result = dbquery("CREATE TABLE ".$db_prefix."eps_rangs(
     id smallint(5) NOT NULL auto_increment,
     rang_name varchar(30) NOT NULL,
     rang_points smallint(5) NOT NULL,
     PRIMARY KEY  (id)
      ) ".ENGINE.";");
...
if (isset($_POST['step']) && $_POST['step'] == "6") {
	...
	if ($error == "") {
		...
		$result = dbquery("INSERT INTO ".$db_prefix."admin (admin_rights, admin_image, admin_title, admin_link, admin_page) VALUES ('RSS', 'rss.gif', '".$locale['rss']."', 'rss_parser.php', 1)");
		$result = dbquery("INSERT INTO ".$db_prefix."admin (admin_rights, admin_image, admin_title, admin_link, admin_page) VALUES ('LG', 'm_logs.gif', '".$locale['mlog']."', 'mods_log.php', 2)");
		$result = dbquery("INSERT INTO ".$db_prefix."admin (admin_rights, admin_image, admin_title, admin_link, admin_page) VALUES ('NMP', 'nicks_mod.gif', '".$locale['nicks_mod']."', 'nicks_mod.php', 2)");
		$result = dbquery("INSERT INTO ".$db_prefix."admin (admin_rights, admin_image, admin_title, admin_link, admin_page) VALUES ('LM', 'links.gif', '".$locale['link_manager']."', 'link_manager.php', 3)");
	  $result = dbquery("INSERT INTO ".$db_prefix."admin (admin_rights, admin_image, admin_title, admin_link, admin_page) VALUES ('EP', 'eps.gif', '".$locale['extreme_point']."', 'eps_settings.php', 2)");
    $result = dbquery("INSERT INTO ".$db_prefix."admin (admin_rights, admin_image, admin_title, admin_link, admin_page) VALUES ('VIP', 'vip.gif', '".$locale['vip']."', 'vip.php', 2)");    
		
		$result = dbquery("INSERT INTO ".$db_prefix."eps_points (point_id, point_type, point_ammount) VALUES
    (1, 'Forum', 1),
    (2, 'Shoutbox', 2),
    (3, 'Link', 3),
    (4, 'Article', 4),
    (5, 'News', 5)");
		...
		$result = dbquery("INSERT INTO ".$db_prefix."nicks_mod (nicks_group_id, nicks_color, nicks_prefix, nicks_styles) VALUES('l103', '#FF0000', '@', '')");
		$result = dbquery("INSERT INTO ".$db_prefix."nicks_mod (nicks_group_id, nicks_color, nicks_prefix, nicks_styles) VALUES('l102', '#0066FF', '#', '')");
		
		$result = dbquery("INSERT INTO ".$db_prefix."users (user_name, user_password, user_admin_password, user_email, user_hide_email, user_offset, user_avatar, user_posts, user_threads, user_joined, user_lastvisit, user_ip, user_rights, user_groups, user_level, user_status, user_theme, user_location, user_birthdate, user_aim, user_icq, user_msn, user_delete, user_yahoo, user_web, user_sig, user_help_point, user_rang, user_points, points_normal, points_bonus, points_punishment, user_primarygroup) VALUES ('".$username."', '".md5(md5($password1))."', '".md5(md5($admin_password1))."', '".$email."', '1', '0', '', '0', '', '".time()."', '0', '0.0.0.0', 'A.AC.AD.B.BB.C.CP.DB.DC.D.FQ.F.FR.IM.I.IP.M.N.NC.P.PH.PI.PO.S.SL.S1.S2.S3.S4.S5.S6.S7.SB.SM.SU.UF.UG.U.W.WC.RSS.LG.NMP.LM.EP.VIP', '', '103', '0', 'Default', '', '0000-00-00', '', '', '', '0', '', '', '', '0', '', '0', '0', '0', '0', '')");
    ...
    $result = dbquery("INSERT INTO ".$db_prefix."panels (panel_name, panel_filename, panel_content, panel_side, panel_order, panel_type, panel_access, panel_display, panel_status) VALUES ('".$locale['169']."', 'today_top_user_panel', '', '1', '5', 'file', '0', '0', '1')");
		...
		$result = dbquery("INSERT INTO ".$db_prefix."site_links (link_name, link_url, link_visibility, link_position, link_window, link_order) VALUES ('".$locale['144']."', 'ekipa.php', '0', '1', '0', '11')");
		$result = dbquery("INSERT INTO ".$db_prefix."site_links (link_name, link_url, link_visibility, link_position, link_window, link_order) VALUES ('".$locale['145']."', 'regulamin.php', '0', '1', '0', '12')");
		$result = dbquery("INSERT INTO ".$db_prefix."site_links (link_name, link_url, link_visibility, link_position, link_window, link_order) VALUES ('---', '---', '101', '1', '0', '13')");
		$result = dbquery("INSERT INTO ".$db_prefix."site_links (link_name, link_url, link_visibility, link_position, link_window, link_order) VALUES ('".$locale['140']."', 'user_center/submit.php?stype=l', '101', '1', '0', '14')");
		$result = dbquery("INSERT INTO ".$db_prefix."site_links (link_name, link_url, link_visibility, link_position, link_window, link_order) VALUES ('".$locale['141']."', 'user_center/submit.php?stype=n', '101', '1', '0', '15')");
		$result = dbquery("INSERT INTO ".$db_prefix."site_links (link_name, link_url, link_visibility, link_position, link_window, link_order) VALUES ('".$locale['142']."', 'user_center/submit.php?stype=a', '101', '1', '0', '16')");
		$result = dbquery("INSERT INTO ".$db_prefix."site_links (link_name, link_url, link_visibility, link_position, link_window, link_order) VALUES ('".$locale['143']."', 'user_center/submit.php?stype=p', '101', '1', '0', '17')");
		...
		$result = dbquery("INSERT INTO ".$db_prefix."user_fields (field_name, field_group, field_order) VALUES ('user_delete', '1', '10')");
		...
		$result = dbquery("INSERT INTO ".$db_prefix."user_fields (field_name, field_group, field_order) VALUES ('user_helper', '4', '11')");
			$result = dbquery("INSERT INTO ".$db_prefix."user_fields (field_name, field_group, field_order) VALUES ('user_primarygroup', '3', '12')");
