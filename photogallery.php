...
// Na skróty
$zmienna = basename($_SERVER['PHP_SELF']);
include INCLUDES."/na_skroty/$zmienna";
// Koniec
...
if (isset($_GET['photo_id']) && isnum($_GET['photo_id'])){
	...
	if (!checkgroup($data['album_access'])) {
		...
		echo "<img src='".(isset($photo_thumb) && !empty($photo_thumb) ? $photo_thumb : $photo_file)."' alt='".$data['photo_filename']."' title='".$locale['453']."' style='border:0px' /></a>\n</div>\n";
	...
	if (!dbrows($result)) {
		...
		if (!checkgroup($data['album_access'])) {
			...
			echo "<td rowspan='2' align='center' class='tbl1'>";
			...
			if ($rows) {
				...
				echo "<img src='".IMAGES."page_prev.gif' alt='' style='vertical-align:middle;' border='0' /><a href='javascript:history.go(-1);' title='".$locale['sub_101']."'> ".$locale['sub_102']."</a> \n";
				echo "</td>\n
				...
				while ($data = dbarray($result)) {
					...
					if ($data['photo_thumb1'] && file_exists(PHOTODIR.$data['photo_thumb1'])){
						echo "<img src='".PHOTODIR.$data['photo_thumb1']."' alt='".$data['photo_thumb1']."' title='".$locale['431']."' style='border:0px' />";
					} else {
						echo $locale['432'];
					}
					echo "</a><br /><br />\n<span class='small'>\n";
...
//sub-albums slut
	...
	if ($rows) {
		...
		while ($data = dbarray($result)) {
			...
			echo "</a><br /><br />\n<span class='small'>\n";
			$subalb = dbcount("(album_id)", DB_PHOTO_ALBUMS, "album_sub='".$data['album_id']."'");
			if ($subalb != 0) {
			echo "<strong>".$locale['sub_100'].": ".$subalb."</strong><br/><br/>";
			}else{
			echo "";
			}
			echo $locale['403'].showdate("shortdate", $data['album_datestamp'])."<br />\n";
			echo $locale['404']."<a href='".BASEDIR."profile.php?lookup=".$data['user_id']."'>".$data['user_name']."</a><br />\n";
			echo $locale['405'].dbcount("(photo_id)", DB_PHOTOS, "album_id='".$data['album_id']."'")."</span><br />\n";
			//Reserve for info about how many sub albums in the album
			echo "</td>\n";
			$counter++; $k++;
		}
		echo "</tr>\n</table>\n";
